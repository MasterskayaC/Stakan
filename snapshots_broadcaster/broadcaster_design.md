...
Снэпшот/обновление формируется (или берётся из потокобезопасного источника) **в момент обработки команды** в потоке broadcaster.
...
При обработке `SendSnapshotAll` broadcaster строит snapshot **один раз** и затем отправляет его всем текущим подключённым клиентам (итерация по реестру клиентов).
...
Это отличается от подхода "поставить в очередь N команд `SendSnapshotTo`": при раскладывании на множество команд разные клиенты потенциально получат снэпшоты, построенные в разные моменты времени.
