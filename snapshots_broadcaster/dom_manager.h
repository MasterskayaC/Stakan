...
namespace server {
    ...
    std::atomic<bool> broadcasting_{false};      ///< Flag indicating whether broadcasting is active
         ...
         * @param interval_ms Time in milliseconds between snapshots (default =
         * DEFAULT_INTERVAL)
         ...
         * @param use_test_broadcast If true, uses a test snapshot from
         * TmpSnapshotCreator otherwise uses the current snapshot from the order book
         */
