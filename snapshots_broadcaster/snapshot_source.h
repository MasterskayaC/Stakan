 ...
 * @return A unique pointer owning the newly created ISnapshotSource object
 * TmpSnapshotCreator.
 */
constexpr uint8_t TMP_NUM_OF_SNAPSHOTS = 5;
std::unique_ptr<ISnapshotSource> makeTmpSnapshotCreator(bool is_random = false,
                                                        uint8_t snapshot_count = TMP_NUM_OF_SNAPSHOTS);
